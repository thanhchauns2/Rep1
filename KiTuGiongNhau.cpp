...
int main()
{
	...
	Begin()
	{
		ll a;
		cin>>a;
		ll insert, remove, copy;
		cin>>insert>>remove>>copy;
		vec(ll) C(a+1,0);
		C[1] = insert;
		FOR(i,2,a)
		{
			if (i & 1)
			{
				// C[i] = min(C[i-1] + insert, C[i/2] + copy + insert);
				C[i] = min(C[i-1] + insert, C[(i+1)/2] + copy + remove);
			}
			else C[i] = min(C[i-1] + insert, C[i/2] + copy);
			// if (i % 2 == 0) C[i] = min(C[i-1] + insert, C[i/2] + copy);
   //         else C[i] = min(C[i-1] + insert, C[(i+1)/2] + copy + remove);
		}
		cout<<C[a]<<endl;
	}
}
