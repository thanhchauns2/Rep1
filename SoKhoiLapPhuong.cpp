...
{ 
    ...
    ll totalCubes = preProcessedCubes.size(); 
    for (ll i = 0; i < totalCubes; i++) 
	{ 
        string currCube = preProcessedCubes[i]; 
  
        ll digitsInCube = currCube.length(); 
        ll index = 0; 
        ll digitsInNumber = num.length(); 
        for (ll j = 0; j < digitsInNumber; j++) 
		{ 
            if (num[j] == currCube[index]) index++; 
            if (digitsInCube == index) return currCube;             
        } 
    } 
    return "-1"; 
} 
void findLargestCube(ll n) 
{
