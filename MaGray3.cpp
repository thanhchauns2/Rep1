...
void getPower()
{
	reset(power);
	power[1] = 1;
	FOR(i,2,11) power[i] = power[i-1] * 10;
}
...
void factory()
{
	...
	for(ll i=1; i<10000; i*=2)
	{
		...
		FOR(j,i,i+2*i-1)
		{
			for(ll k = j; k < 10000; k += 4*i)
			{
				factor[k] += power[steps];
			}
		}
		FOR(j,i,min(9999LL,2*i-1)) factor2[j] = steps;
	}
}
...
int main()
{
	...
	Begin()
	{
		...
		FORD(i,a.length()-1,0)
		{
			if (a[i]=='1') convert += two;
			two<<=1;
		}
		ll k = a.length() - factor2[convert];
		while (k--) cout<<0;
		cout<<factor[convert]<<endl;
	}
}
