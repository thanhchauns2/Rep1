...
int main()
{
	...
	Begin()
	{
		...
		FOR(i, 1, a - 1)
		{
			...
			{
				ll m, n;
				while (!mystack.empty() && C[mystack.top()] < C[i])
				{
					m = mystack.top();
					mystack.pop();
				}
		...
		while(!mystack.empty())
		{
			if (root == -1)
			{
				root = mystack.top();
				mystack.pop();
				if (!mystack.empty() && rgt[root] != -1 && rgt[root] == mystack.top())
				{
					mystack.pop();
					mystack.push(root);
					root = rgt[root];
				...
				{
					if (root != -1) cout << C[root] << ' ';
					root = -1;
				}
				continue;
			}
			mystack.push(rgt[root]);
			mystack.push(root);
			root = lft[root];
		}
		cout << endl;
