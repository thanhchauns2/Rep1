...
ll calculate(queue<string> C)
{
	...
	while(!C.empty())
	{
		...
		if (numeric(x)) 
		{
			mystack.push(stoll(x));
//			cout<<numeric(x)<<' '<<x<<endl;
		}
		else 
		{
			ll a = mystack.top();
...
void Process(queue<string> C)
{
	queue<string> ns;
	stack<string> s;
	while(!C.empty())
	{
		string token = C.front();
		C.pop();
		if (numeric(token)) ns.push(token);
		...
		{
			while (!s.empty() && operate(token) <= operate(s.top()))
			{
				string a = s.top();
				s.pop();
				ns.push(a);
...
queue<string> getReady(string s)
{
	queue<string> token;
	token.push(string(1,s[0]));
	FOR(i,1,s.length()-1)
	{
		if (s[i] >= '0' && s[i] <= '9' && (token.back().length() > 1 || token.back()[0] >= '0' && token.back()[0] <= '9'))
		{
			token.back().pb(s[i]);
		}
		else token.push(string(1,s[i]));
	}
	return token;
}
