...
class Graph
{
	...
	public:
	    ...
	    bool isEulerianCycle()
	    {
		    ...
		    for (int i = 0; i < V; i++) if (adj[i].size() != in[i]) return false;
			return true;
		}
	    bool isSC()
	    {
		    ...
		    int n;
		    for (n = 0; n < V; n++) if (adj[n].size() > 0) break;
		    DFSUtil(n, visited);
		    for (int i = 0; i < V; i++) if (adj[i].size() > 0 && visited[i] == false) return false;
...
int main()
{
	...
	Begin()
	{
		...
		G.isEulerianCycle() ? cout << 1 : cout << 0;
		cout<<endl;
	}
}
