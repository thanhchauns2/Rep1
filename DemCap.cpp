...
int main()
{
	...
	Begin()
	{
		...
		int left = 0;
		int right = 0;
		while (left < a && C[left] == 0) left++;
		while (right < b && F[right] == 0)
		{
			right++;
			total += (a - left);
		}
		while (left < a && C[left] == 1) left++;
		while (right < b && F[right] == 1)
		{
			right++;
			total += (a - left);
		}
		int save = right;
		while (left < a && C[left] == 2)
		{
			while (right < b && F[right] <= 4) right++;
			total += (b - right);
			left++;
		}
		right = save;
		while (left < a && C[left] == 3)
		{
			left++;
			total += (b - right - counting);
		}
		while (left < a && right < b)
		{
			if (C[left] >= F[right]) right++;
			else
			{
				total += (b - right);
				left++;
