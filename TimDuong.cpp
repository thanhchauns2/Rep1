...
//        cout<<K.first<<' '<<K.second<<endl;
        ...
        if (mymap[Ready(A.front().first, A.front().second)] == 2) return;
    }
}
...
//    myend[Ready(x2,y2)] = -1;
    ...
    while (!A.empty())
    {
        K = A.front();
        A.pop();
        if (mymap[Ready(K.first, K.second)]) return "YES";
        ...
        if (myend[Ready(A.front().first, A.front().second)] == 2) return "NO";
    }
    return "NO";
}
