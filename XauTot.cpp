...
//	}
	...
	FOR(i, 0, a - 1)
	{
		if (c[i] == 'A')
		{
			ll x = upper_bound(all(C[0]), i) - C[0].begin();
			if (x == C[0].size()) continue;
			ll y = lower_bound(all(C[1]), i) - C[1].begin();
			if (y == C[1].size()) total++;
			else if (C[1][y] < C[0][x]) total++;
			total += (a - 1 - C[0][x]);
//			cout << i << ' ' << total << ' ' << C[0][x] << ' ' << C[1][y] << endl;
		}
		else
		{
			ll x = upper_bound(all(C[1]), i) - C[1].begin();
			if (x == C[1].size()) continue;
			ll y = lower_bound(all(C[0]), i) - C[0].begin();
			if (y == C[0].size()) total++;
			else if (C[0][y] < C[1][x]) total++;
			total += (a - 1 - C[1][x]);
//			cout << i << ' ' << total << ' ' << C[1][x] << ' ' << C[0][y] << endl;
		}
	}
	cout << total << endl;
}
