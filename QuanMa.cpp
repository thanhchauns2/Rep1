...
map<p(ll, ll), ll> start, end1;

p(ll, ll) START, END;

ll R[8] = {-1, -2, -2, -1, 1, 2, 2, 1};

ll C[8] = {-2, -1, 1, 2, 2, 1, -1, -2};
...
void Ready()
{
	...
	while(!A.empty())
	{
		...
		FOR(i, 0, 7)
		{
			if (!valid(x.f + R[i], x.s + C[i])) continue;
			if (start[{x.f + R[i], x.s + C[i]}]) continue;
			start[{x.f + R[i], x.s + C[i]}] = start[x] + 1;
			A.push({x.f + R[i], x.s + C[i]});
		}
		if (start[A.back()] == 7) return;
	}
}
...
ll Get()
{
	...
	while(!A.empty())
	{
		p(ll, ll) x = A.front();
		A.pop();
		if (start[x]) return start[x] + end1[x];
		FOR(i, 0, 7)
		{
			if (!valid(x.f + R[i], x.s + C[i])) continue;
			if (end1[{x.f + R[i], x.s + C[i]}]) continue;
			end1[{x.f + R[i], x.s + C[i]}] = end1[x] + 1;
			A.push({x.f + R[i], x.s + C[i]});
		}
		if (start[A.back()] == 7) return -1;
	}
}
