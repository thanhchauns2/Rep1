...
void PREattachment()
{
	FOR(i, 0, a - 1)
	{
		FOR(j, 0, i - 1)
		{
			FOR(k, 0, 25) ATT[i][j] += min(get1[i][k], get1[j][k]);
		}
	}
//	ll counting = 0;
//	FOR(i,0,25)
//	{
//		counting += min(get1[a][i], get1[b][i]);
//	}
//	return counting;
}

ll attachment(ll a, ll b)
{
	if (a < b) swap(a, b);
	return ATT[a][b];
}

void Try(ll i, ll atchm)
{
	if (i == a)
	{
		if (atchm < attach)
...
//	}
	cin>>a;
	C.resize(a);
	get1.resize(a,factor);
	FOR(i,0,a-1)
	{
		FOR(j,0,25) get1[i][j] = 0;
	}
	FOR(i,0,a-1)
	{
		cin>>C[i];
		FOR(j,0,C[i].size()-1) get1[i][C[i][j] - 'A']++;
	}
	F.resize(a);
	...
	PREattachment();
	Try(0,0);
	(a < 2) ? cout<<0<<endl : cout<<attach<<endl;
//	cout<<attachment(0,2);
}
