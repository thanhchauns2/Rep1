...
void Process(vec(vec(ll)) &C)
{
	...
	FOR(i,0,C.size()-1)
	{
		FOR(j,0,C[0].size()-1)
		{
			if (C[i][j] == 2)
			{
				if (i > 0 && C[i-1][j] == 1 && F[i-1][j] == 1)
				{
					F[i-1][j] = 2;
					count1--;
					token = 1;
				}
				if (i < C.size()-1 && C[i+1][j] == 1 && F[i+1][j] == 1)
				{
					F[i+1][j] = 2;
					count1--;
					token = 1;
				}
				if (j < C[0].size()-1 && C[i][j+1] == 1 && F[i][j+1] == 1)
				{
					F[i][j+1] = 2;
					count1--;
					token = 1;
				}
				if (j > 0 && C[i][j-1] == 1 && F[i][j-1] == 1)
				{
					F[i][j-1] = 2;
					count1--;
					token = 1;
