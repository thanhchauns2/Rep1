...
class Graph
{
	...
	public:
		...
		void Proceed(ll a, bool visited[])
		{
			visited[a] = true;
			ite(list,ll,i);
			for (i = adj[a].begin(); i != adj[a].end(); ++i) if (!visited[*i]) Proceed(*i, visited);
		}
		bool Connected()
		{
			bool visited[V];
			FOR(i1,0,V-1) visited[i1] = false;
			int i;
			for(i = 0; i < V; i++)
			{
		...
		ll Find()
		{
			...
			ll odd = 0;
			FOR(i,0,V-1) if (adj[i].size() & 1) odd++;
			if (odd > 2) return 0;
			return (odd) ? 1 : 2;
		}
};
