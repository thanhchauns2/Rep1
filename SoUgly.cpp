...
void factory()
{
	...
	FOR(i,1,62)
	{
		FOR(j,1,38)
		{
			if (powerOf2[i] > 1e18 / powerOf3[j]) break;
			factor.pb(powerOf2[i] * powerOf3[j]);
			// cout<<powerOf2[i] * powerOf3[j]<<endl;
			FOR(k,1,27)
			{
				if (powerOf2[i] * powerOf3[j] > 1e18 / powerOf5[k]) break;
	...
	FOR(j,1,38)
	{
		FOR(k,1,27)
		{
			if (powerOf3[j] > 1e18 / powerOf5[k]) break;
	...
	FOR(i,1,62)
	{
		FOR(k,1,27)
		{
			if (powerOf2[i] > 1e18 / powerOf5[k]) break;
