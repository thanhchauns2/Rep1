...
int main()
{
	...
	Begin()
	{
		...
		FOR(i, 0, a.length() - 1)
		{
			...
			if (i > 0 && !isdigit(a[i - 1])) x = 1 - x;
			if (x == 0)
			{
				token = token * 10 + a[i] - '0';
			}
			else
			{
				token2 = token2 * 10 + a[i] - '0';
			}
		}
		if (token) C[token2] += token;
		x = 0;
		token = 0;
		token2 = 0;
		FOR(i, 0, b.length() - 1)
		{
			...
			if (i > 0 && !isdigit(b[i - 1])) x = 1 - x;
			if (x == 0)
			{
				token = token * 10 + b[i] - '0';
			}
			else
			{
				token2 = token2 * 10 + b[i] - '0';
			}
		}
		if (token) C[token2] += token;
		ll k;
