...
ll bfs0[MAX][MAX], bfs1[MAX][MAX], bfs2[MAX][MAX];
...
ll n, h, w, currMin, curr, nextX, nextY, result;

p(ll, ll) p, source;

queue<p(ll, ll) > nextq[2] = 
{
    queue<p(ll, ll)  >(),
    queue<p(ll, ll) >()
};

void bfs(ll x, ll y, ll h, ll w, ll dp[MAX][MAX]) 
{
    curr = 0;
    currMin = 0;
    memset(dp, -1, MAX * MAX * sizeof(ll));

    nextq[curr].push(make_pair(x, y));
    dp[x][y] = 0;
    ...
    while (!nextq[0].empty() || !nextq[1].empty()) 
	{
        ...
        for (ll i = 0; i < 4; i++) 
		{
            nextX = p.first + R[i];
            nextY = p.second + C[i];
            if (nextX >= 0 && nextX < h && nextY >= 0 && nextY < w && FLOOR[nextX][nextY] != '*' && dp[nextX][nextY] == -1) 
			{ 
...
//  }
    ...
    bfs(SRC[0].first, SRC[0].second, h + 2, w + 2, bfs0);
    bfs(SRC[1].first, SRC[1].second, h + 2, w + 2, bfs1);
    bfs(SRC[2].first, SRC[2].second, h + 2, w + 2, bfs2);
