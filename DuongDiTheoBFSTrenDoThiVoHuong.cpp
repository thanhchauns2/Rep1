...
queue<ll> Process(ll c, ll d, vec(vec(ll)) C)
{
	queue<pair<queue<ll>, vec(bool)> > K;
	queue<ll> F2;
	F2.push(c);
	vec(bool) F1(a + 1, false);
	F1[c] = true;
	pair<queue<ll>, vec(bool)> F;
	F.first = F2;
	F.second = F1;
	K.push(F);
	while(!K.empty())
	{
		F = K.front();
		ll x = F.first.back();
		if (x == d) return F.first;
		K.pop();
		FOR(i,0,C[x].size() - 1)
		{
			if (F.second[C[x][i]]) continue;
			pair<queue<ll>, vec(bool)> L = F;
			L.second[C[x][i]] = true;
			L.first.push(C[x][i]);
			K.push(L);
		}
