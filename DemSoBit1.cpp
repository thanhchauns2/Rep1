...
//		cout << a << ' ' << l << ' ' << r << ' ' << 0 << endl;
	...
	if (mymap[a] && l >= b && r <= c)
	{
//		cout << a << ' ' << l << ' ' << r << ' ' << map2[a] << endl;
		return map2[a];
	}
	ll x = ((r + 1 - l) / 2);
	ll k = check(a/2, l, l + x - 1) + check(a%2, l + x, r - x) + check(a/2, r - x + 1, r);
	if (l >= b && r <= c)
	{
		map2[a] = k;
		mymap[a] = 1;	
...
int main()
{
	...
	Begin()
	{
		...
		ll a;
		cin >> a >> b >> c;
		ll k = lower_bound(all(F), a) -  F.begin();
		ll answer = check(a, 1, F[k]);\
//		ll total = 0;
//		FOR(i,b-1,c-1) if (answer[i] == '1') total++;
		cout << answer << endl;
	}
}
