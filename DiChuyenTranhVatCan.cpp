...
void Process()
{
    ...
    K.first = x1;
    K.second = y11;
//    mymap[Ready(x1,y11)] = -1;
    A.push(K);
    while (!A.empty())
    {
        K = A.front();
        A.pop();
        ll steps = mymap[Ready(K.first, K.second)];
//        cout<<K.first<<' '<<K.second<<endl;
        FOR(i,K.first+1,a-1)
        {
        	if (C[i][K.second] == 'X') break;
            if (!mymap[Ready(i,K.second)])
            {
                mymap[Ready(i,K.second)] = steps + 1;
                p(ll,ll) token;
                token.first = i;
                token.second = K.second;
                A.push(token);
            }
            // if (C[K.first][i] != 'X' && !mymap[Ready(K.first,i)])
            // {
            //     mymap[Ready(K.first,i)] = steps + 1;
            // }
        }
        FORD(i,K.first-1,0)
        {
        ...
        FOR(i,K.second+1,a-1)
        {
        	if (C[K.first][i] == 'X') break;
            // if (!mymap[Ready(i,K.second)])
            // {
            //     mymap[Ready(i,K.second)] = steps + 1;
            // }
            if (!mymap[Ready(K.first,i)])
            {
                mymap[Ready(K.first,i)] = steps + 1;
                p(ll,ll) token;
                token.first = K.first;
        ...
        if (mymap[Ready(A.back().first, A.back().second)] == 200) return;
    }
}
...
//    myend[Ready(x2,y2)] = -1;
    ...
    while (!A.empty())
    {
        K = A.front();
        A.pop();
        if (mymap[Ready(K.first, K.second)]) return mymap[Ready(K.first, K.second)] + myend[Ready(K.first, K.second)];
        ll steps = myend[Ready(K.first, K.second)];
        FOR(i,K.first+1,a-1)
        {
        	if (C[i][K.second] == 'X') break;
            if (!myend[Ready(i,K.second)])
            {
                myend[Ready(i,K.second)] = steps + 1;
                p(ll,ll) token;
                token.first = i;
                token.second = K.second;
                A.push(token);
        ...
        FOR(i,K.second+1,a-1)
        {
            ...
            //     mymap[Ready(i,K.second)] = steps + 1;
            // }
            if (!myend[Ready(K.first,i)])
            {
                myend[Ready(K.first,i)] = steps + 1;
                p(ll,ll) token;
                token.first = K.first;
                token.second = i;
                A.push(token);
        ...
        if (myend[Ready(A.back().first, A.back().second)] == 200) return -1;
    }
    return -1;
}
