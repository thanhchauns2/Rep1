...
void Result(ll counting)
{
	if (counting == c) F.insert(token);
}

void Try(ll i, ll counting)
{
	FOR(j, from[i-1], right1-1)
	{
		from[i] = j + 1;
		token[i] = C[j];
		if (i == a) Result(counting + C[j]);
		else Try(i + 1, counting + C[j]);
	}
}
...
int main()
{
	...
	Begin()
	{
		...
		token.resize(a + 1);
		from.resize(a + 1);
		left1 = upper_bound(all(C),b) - C.begin();
		right1 = lower_bound(all(C),c) - C.begin();
		from[0] = left1;
		Try(1, 0);
		// cout<<left1<<' '<<right1<<endl;
		vec(vec(ll)) K(F.begin(), F.end());
		cout<<K.size()<<endl;
		if (K.empty()) continue;
		FOR(i,0,K.size()-1)
		{
			FOR(j,1,K[i].size()-1) cout<<K[i][j]<<' ';
			cout<<endl;
		}
	}
}
