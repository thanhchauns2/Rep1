...
int main()
{
	...
	Begin()
	{
		...
		FOR(i, 0, a - 1)
		{
			...
			char k;
			cin >> k;
			k == 'L' ? lft2[x] = y : rgt2[x] = y;
		}
		ll token = 0;
		m(ll,ll)::iterator it;
		for (it = lft1.begin(); it != lft1.end(); ++it)
		{
			if (lft2[it->first] != it->second)
			{
				token = 1;
				break;
			}
		...
		for (it = rgt1.begin(); it != rgt1.end(); ++it)
		{
			if (rgt2[it->first] != it->second)
			{
				token = 1;
				break;
			}
		}
		token == 0 ? cout << 1 : cout << 0;
		cout << endl;
	}
}
